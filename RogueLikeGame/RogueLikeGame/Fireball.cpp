...
CFireBall::CFireBall() {}
CFireBall::CFireBall(float x, float y, char direccion, SDL_Renderer*  m_WindowRenderer, std::vector<CEnemy*> enemigos)
{
	this->x = x;
	this->y = y;
...
void CFireBall::animation() {
	...
	//Velocidad de transici entre frames.
	if (cont > 10) {
		cont = 0;
	}
	cont++;
...
bool CFireBall::colision() {
	...
	int leftA = rectFb.x;
	int rightA = rectFb.x + rectFb.w;
	int topA = rectFb.y;
	int bottomA = rectFb.y + rectFb.h;

	SDL_Rect rectEnemy;
	
	for (int i = 0; i < enemigos.size(); i++)
	{
		rectEnemy.x = enemigos.at(i)->get_mPosX();
		rectEnemy.y = enemigos.at(i)->get_mPosY();
		rectEnemy.w = 51;
		rectEnemy.h = 35;
		...
		leftB = rectEnemy.x;
		rightB = rectEnemy.x + rectEnemy.w;
		topB = rectEnemy.y;
		bottomB = rectEnemy.y + rectEnemy.h;
